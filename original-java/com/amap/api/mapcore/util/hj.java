package com.amap.api.mapcore.util;

import android.content.Context;
import com.amap.api.mapcore.util.hd.a;
import com.google.devtools.build.android.desugar.runtime.ThrowableExtension;
import java.io.ByteArrayInputStream;
import java.io.Closeable;
import java.security.PublicKey;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.List;

/* compiled from: Utils */
public class hj {
    static PublicKey a() {
        ByteArrayInputStream byteArrayInputStream;
        try {
            CertificateFactory instance = CertificateFactory.getInstance("X.509");
            byteArrayInputStream = new ByteArrayInputStream(ft.b(""));
            try {
                PublicKey publicKey = ((X509Certificate) instance.generateCertificate(byteArrayInputStream)).getPublicKey();
                try {
                    a((Closeable) byteArrayInputStream);
                    return publicKey;
                } catch (Throwable th) {
                    ThrowableExtension.printStackTrace(th);
                    return publicKey;
                }
            } catch (Throwable th2) {
                th = th2;
            }
        } catch (Throwable th3) {
            th = th3;
            byteArrayInputStream = null;
            a((Closeable) byteArrayInputStream);
            throw th;
        }
    }

    public static int a(String str, String str2) {
        int i = 0;
        try {
            String[] split = str.split("\\.");
            String[] split2 = str2.split("\\.");
            int min = Math.min(split.length, split2.length);
            for (int i2 = 0; i2 < min; i2++) {
                i = split[i2].length() - split2[i2].length();
                if (i != 0) {
                    break;
                }
                i = split[i2].compareTo(split2[i2]);
                if (i != 0) {
                    break;
                }
            }
            if (i != 0) {
                return i;
            }
            return split.length - split2.length;
        } catch (Exception e) {
            gc.a((Throwable) e, "Utils", "compareVersion");
            return -1;
        }
    }

    static void a(List<hh> list) {
        int i = 0;
        while (true) {
            int i2 = i;
            if (i2 < list.size() - 1) {
                int i3 = i2 + 1;
                while (true) {
                    int i4 = i3;
                    if (i4 >= list.size()) {
                        break;
                    }
                    hh hhVar = (hh) list.get(i2);
                    hh hhVar2 = (hh) list.get(i4);
                    if (a(hhVar2.e(), hhVar.e()) > 0) {
                        list.set(i2, hhVar2);
                        list.set(i4, hhVar);
                    }
                    i3 = i4 + 1;
                }
                i = i2 + 1;
            } else {
                return;
            }
        }
    }

    static boolean b(String str, String str2) {
        String a = fu.a(str);
        if (a == null || !a.equalsIgnoreCase(str2)) {
            return false;
        }
        return true;
    }

    static boolean a(Context context, gl glVar, String str, fx fxVar) {
        return a(glVar, str, hd.a(context, str), fxVar);
    }

    static boolean a(gl glVar, String str, String str2, fx fxVar) {
        hh a = a.a(glVar, str);
        if (a == null || !fxVar.b().equals(a.d()) || !b(str2, a.b())) {
            return false;
        }
        return true;
    }

    static void a(Throwable th, String str, String str2) {
        gc.a(th, str, str2);
    }

    static void a(Closeable closeable) {
        if (closeable != null) {
            try {
                closeable.close();
            } catch (Throwable th) {
                ThrowableExtension.printStackTrace(th);
            }
        }
    }
}
