...
public class BeanUserLocation {
    ...
    public BeanUserLocation(MitalkProtos.LocationSync locationSync) {
        ...
        try {
            this.name = new String(locationSync.getUserName().toByteArray(), "GB2312");
    ...
    public BeanUserLocation(SealProtos.SEAL_LocationInfo sEAL_LocationInfo) {
        ...
        try {
            this.name = new String(sEAL_LocationInfo.getUserName().toByteArray(), "GB2312");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    public BeanUserLocation(SharkProtos.SHARK_LocationInfo sHARK_LocationInfo) {
        ...
        try {
            this.name = new String(sHARK_LocationInfo.getUserName().toByteArray(), "GB2312");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    public BeanUserLocation(BleProtos.LocationInfo locationInfo) {
        this.userID = String.valueOf(locationInfo.getUserId());
        try {
            this.name = new String(locationInfo.getUserName().toByteArray(), "GB2312");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        ...
        this.time = locationInfo.getTimestamp();
        this.freq = locationInfo.getUserFreq();
    }

    public BeanUserLocation(ShareLocationModel shareLocationModel, boolean z) {
        if (z) {
            this.userID = String.valueOf(shareLocationModel.getUserId());
            this.imgUrl = shareLocationModel.getImgUrl();
            this.name = shareLocationModel.getName();
            this.longitude = shareLocationModel.getLongitude();
            this.latitude = shareLocationModel.getLatitude();
            this.altitude = shareLocationModel.getAltitude();
            this.time = shareLocationModel.getTime();
            this.freq = shareLocationModel.getFreq();
            this.fromDevType = shareLocationModel.getFromDevType();
        }
    }
