...
public class GMSSSigner implements MessageSigner {
    ...
    private void initSign() {
        ...
        if (gMSSPrivateKeyParameters.isUsed()) {
            ...
            System.arraycopy(gMSSPrivateKeyParameters.getCurrentSeeds()[this.numLayer - 1], 0, bArr2, 0, this.mdLength);
            ...
            for (int i = 0; i < this.numLayer; i++) {
                this.currentAuthPaths[i] = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{currentAuthPaths2[i].length, this.mdLength});
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        while (i >= 0) {
            ...
            int i4 = signatureLength + i3;
            byte[] Verify = winternitzOTSVerify.Verify(bArr, bArr3);
            if (Verify == null) {
                System.err.println("OTS Public Key is null in GMSSSignature.verify");
                return false;
            }
            byte[][] bArr4 = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.gmssPS.getHeightOfTrees()[i], this.mdLength});
            i2 = i4;
            for (byte[] arraycopy : bArr4) {
                System.arraycopy(bArr2, i2, arraycopy, 0, this.mdLength);
                i2 += this.mdLength;
            }
