...
public class WebSocketServerHandshaker07 extends WebSocketServerHandshaker {
    ...
    public WebSocketServerHandshaker07(String str, String str2, boolean z, long j) {
    ...
    public ChannelFuture handshake(Channel channel, HttpRequest httpRequest) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(String.format("WS Version 7 Server Handshake key: %s. Response: %s.", new Object[]{header, base64}));
        }
        ...
        write.addListener(new ChannelFutureListener() {
            public void operationComplete(ChannelFuture channelFuture) {
                ...
                ((HttpRequestDecoder) pipeline.get(HttpRequestDecoder.class)).replace("wsdecoder", new WebSocket07FrameDecoder(true, WebSocketServerHandshaker07.this.allowExtensions, WebSocketServerHandshaker07.this.getMaxFramePayloadLength()));
                pipeline.replace(HttpResponseEncoder.class, "wsencoder", (ChannelHandler) new WebSocket07FrameEncoder(false));
            }
        });
        return write;
    }
